...
def test_dicom_to_nifti_select_suffix(dummy_mixedfmap_dicom):
    ...
    nifti_gz_x_imaginary = Nifti_Gzip_Bids.convert(dummy_mixedfmap_dicom, file_postfix="_imaginary")
    nifti_gz_x_real = Nifti_Gzip_Bids.convert(dummy_mixedfmap_dicom, file_postfix="_real")
...
def test_dicom_to_nifti_with_jq_edit(dummy_t1w_dicom):
    ...
    nifti_gz_x = Nifti_Gzip_Bids.convert(dummy_t1w_dicom, side_car_jq=".EchoTime *= 1000")
...
def test_dicom_to_niftix_with_fslgrad(dummy_dwi_dicom):
    ...
    bvec_mags = [(v[0] ** 2 + v[1] ** 2 + v[2] ** 2) for v in nifti_gz_x_fsgrad.grads.bvecs if any(v)]

    assert all(b in (0.0, 3000.0) for b in nifti_gz_x_fsgrad.grads.bvals)
